...
from akari.z3solver import z3solve, z3unique, z3solves
    ...
    for dx in [0, 1, -1, 2, -2]:
        for dy in [0, 1, -1, 2, -2]:
            if not (0 <= y+dy < len(puzzle) and 0 <= x+dx < len(puzzle[0])):
                count += 0.10
            elif puzzle[y + dy][x + dx] != constants.N:
...
def _random_location(puzzle, poss, solutions=None):
    ...
    choices = ([(x, y) for (x, y) in poss if puzzle[y][x] == constants.N])
...
def _remove_all_numbers(puzzle, poss):
    ...
    for x, y in [(x, y) for (x, y) in poss if puzzle[y][x] != constants.N]:
        puzzle[y][x] = constants.B
...
def _place_numbers(puzzle, poss):
    ...
    for x, y in [(x, y) for (x, y) in poss if puzzle[y][x] == constants.B]:
        count = 0
        for dx, dy in constants.DIRS:
...
def generate(height, width, start=None, step=None, symmetry=None, seed=None):
    puzzle = [[constants.N for _ in range(width)] for _ in range(height)]
